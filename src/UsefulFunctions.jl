...
# function t̃_To_tStar(T̃ᵣ, N, ℰ, Ω, 𝒜, C_b, S_b, k₁, k₂, k₃, k_Ca, k_Cd, k_Sa, k_Sd)
...
function 𝒫star₅₀Analytic(h₀, h_C, h_S, k₁, k₂, k₃, k₄, 𝒜, 𝒮, 𝒞, ℰ, N, ϕ) 
    u = h₀/h_C
    λ = h_C/h_S
    ζ = (2*k₂*𝒜)/(k₃*𝒮)
    γ = (2*k₂*𝒜)/(k₁*𝒞)
    ...
    return ((k₁*𝒞*ℰ)/(4.0*𝒜*N*ϕ))*F
end

# Eq 50
function 𝒫star₅₀Numeric(N, k₁, k₂, k₃, 𝒞, ℰ, 𝒮, h₀, k_Ca, k_Cd, k_Sa, k_Sd, 𝒜, T̃ᵣ₅₀)
    h_C = 2*k_Ca/k_Cd
    h_S = 2*k_Sa/k_Sd
    ζ = (2*k₂*𝒜)/(k₃*𝒮)
    a = ((k₁*𝒞 )^2)*ℰ/(k₃*𝒮)
...
function T̃ᵣ₅₀Analytic(𝒞, 𝒮, ϕ, h₀, h_C, h_S, k₁, k₂, k₃, k₄, 𝒜, N, ν₀, t̃₀) 
...
# function 𝓟starUniform(𝒞, ℰ, 𝒮, ϕ, N, k₁, k₂, K₃, K₄, 𝒜, h₀, h_C, h_S)
#     Δ = k₁*𝒞/(2.0*k₂*𝒜)
#     return (𝒞/(4*ϕ))*(k₁*ℰ/(𝒜*N))* (h₀/((h₀+h_C)*(h₀+h_C*(1+Δ)))) * ((𝒮*Δ*K₃/𝒞 - K₄ - K₄*h₀/h_S)/(1 + 𝒮*Δ*K₃/𝒞 + h₀/h_S) )
# end
