...
# \ref{eq:Mstar}
function M_star_ϕ(u, W, dims, dν, hᵥ, α_C, 𝓒, Ω, ϕ)
    M̃ = M_tilde(u, W, dims, dν, hᵥ)
    Mϕ = dν*sum(M̃[floor(Int64, ϕ*dims[1]) : dims[1]])
    prefactor = α_C*𝓒/(π*(1+α_C))
    return prefactor*Mϕ
end

function P_star(u, W, dims, dν, hᵥ, α_C, C_b, Ω, ϕ, Ωperp, k₁, 𝓔, TᵣStar)
    return M_star_ϕ(u, W, dims, dν, hᵥ, α_C, C_b, Ω, ϕ)/TᵣStar 
end

function Pstar₅₀Analytic(h₀, h_C, h_S, k₁, k₂, k₃, k₄, Ωperp, 𝓢, 𝓒, 𝓔, N, ϕ) 
    u = h₀/h_C
    λ = h_C/h_S
    ζ = (2*k₂*Ωperp)/(k₃*𝓢)
    γ = (2*k₂*Ωperp)/(k₁*𝓒)
