...
function hFromData(dims; cisternaSeriesID=1)
    ...
    xs   = collect(range(0.0, xMaxInferred, dims[2])) # Positions of discretised vertices in space
...
function hFromFunction(dims; xMax=100.0, μxh = 50.0, σxh = 10.0)
    hFun(x) = 0.1+exp(-(x-μxh)^2/σxh^2)
    xs   = collect(range(0.0, xMax, dims[2])) # Positions of discretised vertices in space
    mat_h = zeros(dims...)
    for j=1:dims[2]
        # mat_h[:, j] .= hFun(xs[j])
        selectdim(mat_h, 2, j) .= hFun(xs[j])
