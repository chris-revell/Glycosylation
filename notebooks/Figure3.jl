...
# folderName = "25-02-05-15-25-56_K₂=0.3_K₄=1.0_T̃ᵣ=0.385_differencing=centre_nSpatialDims=1_α_C=5.0_β=70.0_𝒟=2040.0"
...
for x=2:4
    ...
    axesVec[end].xticks = (0.0:0.5:1.0, [L"0.0", L"0.5", L"1.0"])
    axesVec[end].yticks = (0.0:sqrt(π)/2.0:sqrt(π), [L"0.0", L"\sqrt{\pi}/2", L"\sqrt{\pi}"])
...
for x=2:4  
    ...
    xlims!(axesVec[end], (0.0,1.0))
    ylims!(axesVec[end], (0.0,40.0))
    axesVec[end].xticks = (0.0:0.5:1.0, [L"0.0", L"0.5", L"1.0"])
    axesVec[end].yticks = (0.0:20.0:40.0, [L"0.0", L"20.0", L"40.0"])
end
...
rowsize!(fig.layout, 2, Relative(0.0051))
rowsize!(fig.layout, 4, Relative(0.0051))
rowsize!(fig.layout, 6, Relative(0.0051))
rowsize!(fig.layout, 8, Relative(0.0051))
