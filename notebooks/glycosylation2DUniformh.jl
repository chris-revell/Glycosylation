...
k₁   = 1.1   # Complex formation forward reaction rate 
k₂   = 0.9   # Complex dissociation reverse reaction rate 
k₃   = 1.1   # Product formation
k₄   = 0.9  # Product dissociation 
E_0 = 1.0
𝓒 = 100.0
𝓢 = 1000.0
...
C_b = 100.0 #𝓒/Ω                     # Initial monomer bulk concentration 
S_b = 1000.0 #𝓢/Ω                     # Initial substrate mass
...
for h₀ in h₀s
    ...
    push!(hᵥs, hᵥ)
    push!(Ωs, h₀*Ωperp)
    push!(α_Cs, (k_Cd*Ωs[end])/(2*k_Ca*Ωperp))
    ...
    sol = solve(prob, Vern9(), saveat=Tᵣ)

    push!(uFinals, sol.u[end])
...
for i=1:length(uFinals)
    push!(Pstars, P_star(uFinals[i], W, ghostVertexMaskVec, dims, hᵥs[i], ϕ, α_Cs[i], C_b, Ωs[i], Tᵣstar))
end
lines!(ax, h₀s, Pstars)
ax.xlabel = "h₀"
ax.ylabel = L"𝓟^*"
